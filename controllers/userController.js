...
export const register = catchAsyncError(async(req,res,next)=>{
    try {
        ...
        if(role ==="Job Seeker" && (!firstNiche || !secondNiche || !thirdNiche)){
            return next(new ErrorHandler("Plese provide your preffered job niches. ",400));
        }
...
export const getUser = catchAsyncError(async(req,res,nex)=>{
    const user = req.user;
    res.status(200).json({
        success: true,
        user,
...
export const updatePassword = catchAsyncError(async(req,res,next)=>{
    ...
    if(req.body.newPassword !== req.body.confirmPassword){
        return next(new ErrorHandler("Both password should be same.",400));
    }
